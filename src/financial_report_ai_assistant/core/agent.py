...
10. 【多期数据年份标注】当财务数据包含多期时，必须从表头行读取每列对应的年份，严格按表头标注匹配数值，不要依赖列的位置。回答时必须准确标注每个数值对应的年份，禁止将两年数据互换
...
- 当数据包含多期时，必须从表头行读取每列对应的年份，严格按表头标注匹配数值，不要依赖列的位置。回答时必须准确标注每个数值对应的年份，禁止将两年数据互换
