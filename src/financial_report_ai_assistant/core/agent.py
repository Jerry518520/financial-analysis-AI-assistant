...
def tool_calculate_roe(net_income: float, equity: float, beginning_equity: float = None) -> str:
    ...
    ⚠️ 口径要求（必须严格遵守）：
    - net_income（分子）必须是"归属于上市公司股东的净利润"（归母净利润），不是扣除非经常性损益后的净利润
    - equity（分母）必须是"归属于上市公司股东的净资产"（归母净资产），不是总资产减总负债的全部所有者权益
    - 如有期初归母净资产也请传入，系统自动使用平均口径

    注意：此工具使用简单平均法（(期初+期末)/2），而非加权平均法。
    如果财报中已直接给出"加权平均净资产收益率"的数值，应直接引用该数值，不要调用此工具。"""
...
⚠️ 重要规则（按优先级排列，靠前规则优先于靠后规则）：

- 【最高优先级 - 直接引用优先】如果背景信息中直接给出了用户询问的指标数值（如加权平均净资产收益率、每股收益EPS、每股净资产、市盈率、速动比率、流动比率等），直接引用该数值，不要尝试用公式计算。只有当背景信息中确实没有直接给出该指标时，才调用计算工具。此规则优先级最高，覆盖以下所有衍生指标计算规则
...
- 【衍生指标计算】以下指标通常需要计算，但前提是背景信息中未直接给出对应数值（如上面的直接引用优先规则所述）：
  ...
  - ROE = 归母净利润 / 平均归母净资产（使用 tool_calculate_roe）。分子必须用"归属于上市公司股东的净利润"（不是扣非净利润），分母必须用"归属于上市公司股东的净资产"（不是总资产−总负债的全部所有者权益）。但如果背景信息中已直接给出"加权平均净资产收益率"或"净资产收益率"则直接引用，不要调用工具
...
- 【多期数据年份标注】当财务数据包含多期（如本期/上期、2025年/2024年）时：
  - 【最优先】从表头行读取每列对应的年份，严格按表头标注匹配数值，不要依赖列的位置
  - 【降级】如果数据中无表头行（原始文本格式），从上下文推断年份顺序（如"2025年12月31日"出现在数据前面则为本期），无法确定时按"前面的数值为较新年份"的惯例处理
...
3. 【绝对禁止】禁止输出"让我先计算"、"我来分析"、"需要先"、"让我调用工具"等中间推理过程或计算步骤，直接给出最终结果
...
4. 只回答用户问的问题，不要主动扩展分析、建议、趋势解读，除非用户明确要求
5. 使用 Markdown 格式，但不要过度使用标题和分节——简单问题直接用段落回答即可
...
9. 【直接引用优先】如果背景信息中直接给出了用户询问的指标数值（如加权平均净资产收益率、每股收益EPS、每股净资产、速动比率、流动比率等），直接引用该数值，不要尝试用公式重新计算。即使该指标属于衍生指标，只要财报中已有数值，就禁止重新计算
10. 【多期数据年份标注】当财务数据包含多期时，必须从表头行读取每列对应的年份，严格按表头标注匹配数值，不要依赖列的位置。如果无表头行，从上下文推断年份顺序，无法确定时按"前面的数值为较新年份"处理。回答时必须准确标注每个数值对应的年份，禁止将两年数据互换
11. 【数据完整性】当背景信息中存在多个匹配项时（如多年数据、分板块数据、多个期间对比），必须全部列出，不得只挑其中一个回答
12. 【绝对禁止】回答中严禁出现以下任何表述：
    - "在历史对话中"
    - "已经调用过...工具"
    - "tool_xxx" 等工具名称
    - "工具调用"、"计算工具"、"使用工具"等描述调用过程的词汇
    - "之前计算过"等描述历史过程的词汇
    - "让我先计算"、"让我调用"、"我来分析"、"需要先"、"首先让我"等中间推理过程
    - 任何描述思考步骤、计算过程、工具调用过程的内容——直接给出最终结果
...
6. 使用 Markdown 格式，但不要过度使用标题和分节——简单问题直接用段落回答即可
7. 只回答用户问的问题，不要主动扩展分析、建议或趋势解读，除非用户明确要求
8. 【绝对禁止】禁止输出"让我先计算"、"我来分析"、"需要先"等中间推理过程，直接给出最终结果
...
- 如果背景信息中直接给出了用户询问的指标数值（如加权平均净资产收益率、每股收益EPS、每股净资产、速动比率、流动比率等），直接引用，不要尝试用公式计算。即使该指标属于衍生指标，只要财报中已有数值，就禁止重新计算
- 当数据包含多期时，必须从表头行读取每列对应的年份，严格按表头标注匹配数值，不要依赖列的位置。如果无表头行，从上下文推断年份顺序，无法确定时按"前面的数值为较新年份"处理。回答时必须准确标注每个数值对应的年份，禁止将两年数据互换
