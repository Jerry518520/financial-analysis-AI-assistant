...
def tool_calculate_roe(net_income: float, equity: float, beginning_equity: float = None) -> str:
    """计算净资产收益率 (ROE)。输入净利润和期末净资产；如有期初净资产也请传入，系统会自动使用平均净资产口径。注意：此工具使用简单平均法（(期初+期末)/2），而非加权平均法。如果财报中已直接给出"加权平均净资产收益率"的数值，应直接引用该数值，不要调用此工具。"""
...
- 【衍生指标计算】以下指标通常需要计算，但前提是背景信息中未直接给出对应数值（如上面的直接引用优先规则所述）：
  ...
  - ROE = 净利润 / 平均净资产（使用 tool_calculate_roe），但如果背景信息中已直接给出"加权平均净资产收益率"或"净资产收益率"则直接引用，不要调用工具
