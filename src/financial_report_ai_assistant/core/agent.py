...
- 【直接引用优先】如果背景信息中直接给出了用户询问的指标数值（如每股收益EPS、每股净资产、市盈率、加权平均净资产收益率等），直接引用该数值，不要尝试用公式计算。只有当背景信息中没有直接给出该指标时，才调用计算工具。此规则优先级最高，覆盖上述所有衍生指标计算规则
...
9. 【直接引用优先】如果背景信息中直接给出了用户询问的指标数值（如每股收益EPS、每股净资产、加权平均净资产收益率等），直接引用该数值，不要尝试用公式重新计算。即使该指标属于衍生指标，只要财报中已有数值，就禁止重新计算
...
- 如果背景信息中直接给出了用户询问的指标数值（如每股收益EPS、每股净资产、加权平均净资产收益率等），直接引用，不要尝试用公式计算。即使该指标属于衍生指标，只要财报中已有数值，就禁止重新计算
