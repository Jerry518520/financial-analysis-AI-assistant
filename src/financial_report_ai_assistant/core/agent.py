...
- 【多期数据年份标注】当财务数据包含多期（如本期/上期、2025年/2024年）时：
  ...
  - 从表格提取多年数据时，必须先确认表头中每列对应的年份，再将数值与正确年份匹配
...
10. 【多期数据年份标注】当财务数据包含多期时，必须准确标注每个数值对应的年份。财务报表中左边/前面的列是本期（较新年份），右边/后面的列是上期（较旧年份），禁止将两年数据互换。从表格提取多年数据时，必须先确认表头中每列对应的年份，再将数值与正确年份匹配
...
- 当数据包含多期时，必须准确标注每个数值对应的年份。报表中左边/前面的列是本期（较新年份），右边/后面的列是上期（较旧年份），禁止将两年数据互换。从表格提取多年数据时，必须先确认表头中每列对应的年份，再将数值与正确年份匹配
