...
def call_chat_api(prompt, context_text):
    """统一的聊天 API 调用逻辑"""
    ...
    if res.status_code == 200:
        ...
        return ai_msg, source_page, recommended
    else:
        return "服务器错误", None, None
    ...
    if 'result' in st.session_state:
        ...
        for idx, msg in enumerate(st.session_state.messages):
            with st.chat_message(msg["role"]):
                ...
                # 如果是 assistant 消息，显示溯源图片和推荐问题
                if msg["role"] == "assistant":
                    # 显示溯源图片
                    if msg.get("source_page"):
                    ...
                    # 显示推荐问题（使用动态 key 避免冲突）
                    if msg.get("recommended"):
                        st.markdown("**💡 你可能还想问：**")
                        rec_cols = st.columns(3)
                        for i, q in enumerate(msg["recommended"]):
                            with rec_cols[i]:
                                if st.button(f"📝 {q}", key=f"rec_{idx}_{i}", use_container_width=True):
                                    st.session_state.pending_question = q
                                    st.rerun()

        # ========== 处理 pending_question（预设/推荐按钮触发）==========
        if "pending_question" in st.session_state:
            ...
            with st.chat_message("assistant"):
                with st.spinner("DeepSeek 正在思考..."):
                    try:
                        ai_msg, source_page, recommended = call_chat_api(prompt, context_text)
                        st.markdown(ai_msg)
                        ...
                        if source_page:
                            try:
                                st.image(f"{API_URL}/highlight?page={source_page}&x=0&y=0&w=600&h=800", caption=f"📄 来源：第 {source_page} 页")
                            except Exception:
                                pass
